...
export class SmartAnalysisDetailPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getVariablesInfoService.getVariablesInfoService(this.locationCustomNameStation).subscribe((res) => {
      ...
      console.log('windDescription'+res.wind.Fuzzyfication.description);
      ...
      console.log('arrayColor'+res.uvindex['all_iuv'][0]);
      ...
      this.arrayCa = res.uvindex['all_iuv'][6];
        // se realiza  la opcion  de valida si llega vacio, el cual deja si falla? y el signo ! es vacio
      if(!!this.arrayCa?.length){
   ...
   // let color = 'black' this.color
    ...
    var data = {
      ...
      datasets: [{
        ...
        backgroundColor: !!this.arrayColor?.length?this.arrayColor:'#77a345',
        borderWidth: 1,
        cutout: '70%'
