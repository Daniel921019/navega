...
@Component({
  selector: 'app-presion-atmosferica',
  templateUrl: './presion-atmosferica.page.html',
  styleUrls: ['./presion-atmosferica.page.scss'],
})
export class PresionAtmosfericaPage implements OnInit {
  ...
  async getSensorId(){
    ...
    try{
      this.sensor_id = await this.dataTransfer.getSensorIdFromApiRest("CAPHZZ01");
    }catch(err){
      console.log(err);
    }
  }
    ...
    if(this.station_value && this.month_value && this.year_value){
      ...
      sensor_data.then(res => {
        ...
        if(response.length > 0){
          ...
          this.dataPresentation.drawingChart('Valores diarios de la Presión Atmosférica - ' + this.month_value + ' ' + this.year_value, response, this.line_chart.nativeElement, 'line',
            'Presión atmosférica por día (Promedio)', this.month_value, this.year_value, 'Presión atmosférica(mb)',
            '#0012FF', '#0012FF');

          this.dataPresentation.drawingTrendChart('Tendencia de los Valores diarios de la Presión Atmosférica - ' + this.month_value + ' ' + this.year_value, response, this.trend_chart.nativeElement,
            'Tendencia de la Presión atmosférica por día', this.month_value, this.year_value, 'Presión atmosférica(mb)',
            '#D38300', '#744800');
          ...
          this.getStatistics(this.sensor_id.sensor_id, station_id.station_id, month_number + 1 + "", month_max_days.max_days, this.year_value).then(res2 => {
            ...
            this.data_table = [
              ...
              {"caption": "Valor máximo mensual", "data": this.statistics.max_data + " mb"},
              {"caption": "Valor mínimo mensual", "data": this.statistics.min_data + " mb"},
              {"caption": "Promedio mensual", "data": this.statistics.avg_data + " mb"},
