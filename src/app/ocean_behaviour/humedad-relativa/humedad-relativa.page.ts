...
@Component({
  ...
  templateUrl: './humedad-relativa.page.html',
  styleUrls: ['./humedad-relativa.page.scss'],
})
export class HumedadRelativaPage implements OnInit {
  ...
  async getSensorId(){
    ...
    try{
      this.sensor_id = await this.dataTransfer.getSensorIdFromApiRest("CRELSS01");
    }catch(err){
      console.log(err);
    }
  }
    ...
    if(this.station_value && this.month_value && this.year_value){
      ...
      sensor_data.then(res => {
        ...
        if(response.length > 0){
          ...
          this.dataPresentation.drawingChart('Valores diarios de la Humedad Relativa - ' + this.month_value + ' ' + this.year_value, response, this.line_chart.nativeElement, 'line',
            'Humedad relativa por día (Promedio)', this.month_value, this.year_value, 'Humedad relativa(%)',
            '#00FF0E', '#00CE0B');

          this.dataPresentation.drawingTrendChart('Tendencia de los Valores diarios de la Humedad Relativa - ' + this.month_value + ' ' + this.year_value, response, this.trend_chart.nativeElement,
            'Tendencia de la Humedad relativa por día', this.month_value, this.year_value, 'Humedad relativa(%)',
            '#D38300', '#744800');
          ...
          this.getStatistics(this.sensor_id.sensor_id, station_id.station_id, month_number + 1 + "", month_max_days.max_days, this.year_value).then(res2 => {
            ...
            this.data_table = [
              ...
              {"caption": "Valor máximo mensual", "data": this.statistics.max_data + " %"},
              {"caption": "Valor mínimo mensual", "data": this.statistics.min_data + " %"},
              {"caption": "Promedio mensual", "data": this.statistics.avg_data + " %"},
