...
@Component({
  selector: 'app-temperatura-aire',
  templateUrl: './temperatura-aire.page.html',
  styleUrls: ['./temperatura-aire.page.scss'],
})
export class TemperaturaAirePage implements OnInit {
  ...
  async getSensorId(){
    ...
    try{
      this.sensor_id = await this.dataTransfer.getSensorIdFromApiRest("CTMPZZ01");
    }catch(err){
      console.log(err);
    }
  }
    ...
    if(this.station_value && this.month_value && this.year_value){
      ...
      sensor_data.then(res => {
        ...
        if(response.length > 0){
          ...
          this.dataPresentation.drawingChart('Valores diarios de la Temperatura del Aire - ' + this.month_value + ' ' + this.year_value, response, this.line_chart.nativeElement, 'line',
            'Temperatura del aire por día (Promedio)', this.month_value, this.year_value, 'Temperatura del aire(°C)',
            '#FF0000', '#C20000');

          this.dataPresentation.drawingTrendChart('Tendencia de los Valores diarios de la Temperatura del Aire - ' + this.month_value + ' ' + this.year_value, response, this.trend_chart.nativeElement,
            'Tendencia de la Temperatura del aire por día', this.month_value, this.year_value, 'Temperatura del aire(°C)',
            '#D38300', '#744800');
          ...
          this.getStatistics(this.sensor_id.sensor_id, station_id.station_id, month_number + 1 + "", month_max_days.max_days, this.year_value).then(res2 => {
            ...
            this.data_table = [
              ...
              {"caption": "Valor máximo mensual", "data": this.statistics.max_data + " °C"},
              {"caption": "Valor mínimo mensual", "data": this.statistics.min_data + " °C"},
              {"caption": "Promedio mensual", "data": this.statistics.avg_data + " °C"},
