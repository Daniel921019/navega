...
@Component({
  selector: 'app-nivel-del-mar',
  templateUrl: './nivel-del-mar.page.html',
  styleUrls: ['./nivel-del-mar.page.scss'],
})
export class NivelDelMarPage implements OnInit {
  ...
  async getSensorId(){
    ...
    try{
      this.sensor_id = await this.dataTransfer.getSensorIdFromApiRest("ASLVRADR");
    }catch(err){
      console.log(err);
    }
  }
    ...
    if(this.station_value && this.month_value && this.year_value){
      ...
      sensor_data.then(res => {
        ...
        if(response.length > 0){
          ...
          this.dataPresentation.drawingChart('Valores diarios del Nivel del Mar - ' + this.month_value + ' ' + this.year_value, response, this.line_chart.nativeElement, 'line',
            'Nivel del mar por día (Promedio)', this.month_value, this.year_value, 'Nivel del mar(m)',
            '#0099FF', '#006FB9');

          this.dataPresentation.drawingTrendChart('Tendencia de los Valores diarios del Nivel del Mar - ' + this.month_value + ' ' + this.year_value, response, this.trend_chart.nativeElement,
            'Tendencia del Nivel del mar por día', this.month_value, this.year_value, 'Nivel del mar(m)',
            '#D38300', '#744800');
          ...
          this.getStatistics(this.sensor_id.sensor_id, station_id.station_id, month_number + 1 + "", month_max_days.max_days, this.year_value).then(res2 => {
            ...
            this.data_table = [
              ...
              {"caption": "Valor máximo mensual", "data": this.statistics.max_data + " m"},
              {"caption": "Valor mínimo mensual", "data": this.statistics.min_data + " m"},
              {"caption": "Promedio mensual", "data": this.statistics.avg_data + " m"},
